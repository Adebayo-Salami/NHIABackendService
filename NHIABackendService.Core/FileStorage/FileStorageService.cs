...
namespace NHIABackendService.Core.FileStorage
{
    ...
    public class FileStorageService : IFileStorageService
    {
        ...
        public void DeleteFile(string path)
        {
            ...
            if (!fileInfo.Exists) throw new ArgumentException(string.Format("File {0} does not exist", path));
        ...
        public void RenameFile(string oldPath, string newPath)
        {
            ...
            if (!sourceFileInfo.Exists) throw new ArgumentException(string.Format("File {0} does not exist", oldPath));

            var targetFileInfo = new FileInfo(MapStorage(newPath));
            if (targetFileInfo.Exists) throw new ArgumentException(string.Format("File {0} already exists", newPath));
        ...
        public string RenameDuplicateFile(string filepath)
        {
            ...
            var oldFileInfo = sourceFileInfo.Name.Split(new[] { "." }, StringSplitOptions.RemoveEmptyEntries);
            var newFilePath = "";
            var counter = 1;
            ...
            {
                counter++;
                newFilePath = folder + oldFileInfo[0] + " (" + counter + ")" + oldFileInfo[1];
            } while (File.Exists(newFilePath));

            //join full directory
            RenameFile(filepath, newFilePath);

            var targetFileInfo = new FileInfo(MapStorage(newFilePath));
        ...
        {
            var fileInfo = new FileInfo(MapStorage(path));
            if (fileInfo.Exists) throw new ArgumentException(string.Format("File {0} already exists", fileInfo.Name));
        ...
        {
            var directoryInfo = new DirectoryInfo(MapStorage(path));
            if (directoryInfo.Exists) throw new ArgumentException(string.Format("Directory {0} already exists", path));
